...
<b>Permit Management System</b>
        ...
        public async Task<IActionResult> workpermitform(ColdWorkPermit vm)
        {
            try
            {
                ...
                {
                    ModelState.AddModelError("Dates", "Please select an End Date after the Start Date.");
                }
        ...
        public async Task<IActionResult> Hotwork(HotWorkPermit model)
        {
...

            {
                ...
                DeptIncharge = model.DeptIncharge,
        ...
        public async Task<IActionResult> ConfinedSpaceEntry(ConfinedSpacePermit model)
        {
            if (model.EndDate <= model.StartDate)
            {
                ModelState.AddModelError("DateTime", "Please select an End Date after the Start Date.");
            }
        ...
        public IActionResult Dashboard()
        {
            ...
            dashboard.AddRange(
                _context.WorkAtHeightPermits.Where(a => a.IsActive).Select(x => new PermitDashboardVM
                {
                    PermitDashBoardId = x.PermitId,
            ...
            dashboard.AddRange(
                _context.ConfinedSpacePermits.Where(a => a.IsActive == true).Select(x => new PermitDashboardVM
                {
                    PermitDashBoardId = x.Id,
            ...
            objpermit.PermitDetailsList = dashboard.OrderByDescending(x => x.CreatedOn).ToList();
            ...
            try
            {
                if (PermitType == "Cold Work")
                {
                    ...
                    if (permitcheck != null)
                    {
                        ...
                        // First Approver
                        if (count >= 1 && first == "Pending")
                        {
                            permitcheck.FirstApproverStatus = Status;
                        }

                        // Second Approver
                        else if (count >= 2 && second == "Pending")
                        {
        ...
        private void UpdatePermitStatus (PermitMaster permit, int count)
        {
            var statuses = new[]
            {
                permit.FirstApproverStatus,
                permit.SecondApproverStatus,
                permit.ThirdApproverStatus,
                permit.FourthApproverStatus
            };

            var releventStatuses = statuses.Take(count).ToList();

            if (releventStatuses.All(x => x == "Rejected"))
                permit.Status = "Rejected";
            else if (releventStatuses.All(x => x == "Approved"))
                permit.Status = "Approved";
            else if (releventStatuses.All(x => x == "Approved" || x == "Rejected"))
        ...
        public JsonResult Delete(string type, int id)
        {
            try
            {
                ...
                {
                    var permit = _context.ConfinedSpacePermits.FirstOrDefault(x => x.Id == id && x.IsActive == true);
